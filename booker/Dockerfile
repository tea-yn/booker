...
COPY --from=build /target/booker-0.0.1-SNAPSHOT.jar booker.jar//前のビルドステージからビルドされたjarファイル(demo.jar)を新しいビルドステージにコピー
EXPOSE 8080 //ポート8080を宣言
ENTRYPOINT ["java", "-jar", "booker.jar"]//実行するファイル(booker.jar)を指定
